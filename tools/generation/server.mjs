...
const artifactRoot = resolve(root, process.env.MAKEWATCH_ARTIFACT_DIR ?? '.makewatch/artifacts/scenes');
const bridge = new GenerationBridgeClient();
const comfy = new ComfyUiClient();
const service = new SceneGenerationService({ bridge, comfy, artifactRoot });
...
function sendJson(request, response, status, payload) {
  ...
  response.end(JSON.stringify({ ok: status >= 200 && status < 300, result: status >= 200 && status < 300 ? payload : undefined, error: status >= 400 ? payload : undefined }));
}
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/health') {
      sendJson(request, response, 200, { service: 'makewatch-generation', mode: 'storyboard-preview' });
      return;
    }
    if (request.method === 'GET' && url.pathname === '/api/provider') {
      sendJson(request, response, 200, await service.providerStatus());
      return;
    }
    if (request.method === 'GET' && url.pathname === '/api/jobs') {
      const limit = Number(url.searchParams.get('limit') ?? '20');
      sendJson(request, response, 200, { jobs: service.list(Number.isInteger(limit) ? limit : 20) });
      return;
    }
    if (request.method === 'POST' && url.pathname === '/api/scenes') {
      const body = await readJson(request);
      const job = await service.startScene(boundedId(body.sceneId, 'sceneId'));
    ...
    if (request.method === 'GET' && jobMatch) {
      sendJson(request, response, 200, { job: service.get(jobMatch[1]) });
      return;
    }

    const artifactMatch = /^\/api\/artifacts\/([A-Za-z0-9-]+)\/([A-Za-z0-9._:-]+)$/.exec(url.pathname);
    if (request.method === 'GET' && artifactMatch) {
      const artifact = service.artifact(artifactMatch[1], artifactMatch[2]);
...
server.listen(port, '127.0.0.1', () => {
  console.log(`[generation] gateway ready at http://127.0.0.1:${port}`);
  console.log(`[generation] artifacts: ${artifactRoot}`);
  void service.providerStatus().then((status) => {
